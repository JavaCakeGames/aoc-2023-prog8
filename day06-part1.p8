...
main {
  ...
  ; Input:
  ubyte[] time = [35, 93, 73, 66]
  uword[] distance = [212, 2060, 1201, 1044]

  ; Example data:
  ;ubyte[] time = [7, 15, 30]
  ;uword[] distance = [9, 40, 200]
  ...
  sub start() {
    ...
    ubyte race
    for race in 0 to len(time) - 1 {

      ubyte buttonTime = 1
      repeat {
        uword dst = buttonTime as uword * (time[race] - buttonTime)
        if dst > distance[race] {
      ...
      buttonTime = time[race] - 1
      repeat {
        dst = buttonTime as uword * (time[race] - buttonTime)
        if dst > distance[race] {
      ...
      if totalPossibilities != 0 {
        totalPossibilities *= maxButtonTime - minButtonTime + 1
        totalPossibilitiesHigh = lsb(math.mul16_last_upper())
      } else {
        totalPossibilities = maxButtonTime - minButtonTime + 1
      }
    ...
    txt.nl()
    floats.print_f(totalPossibilitiesHigh * 65536.0 + totalPossibilities)
    txt.nl()
